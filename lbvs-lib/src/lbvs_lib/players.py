...
class ReturnCodeServer(Player):
    ...
    def code(self, voter_id, ballots):
        ...
        for encrypted_ballot, ballot_proof in ballots:
            r_ = code(self.pk, self.ck, vvk, encrypted_ballot, ballot_proof)
...
class VotingProtocol:
    ...
    def registration_phase(self, voter_size):
        ...
        for i in range(voter_size):
            ...
            # and a set of trusted players compute the return code table
            # The voter gets the return code table
            tables = [ReturnCodeTable.compute_table(self.return_code_server.prf_key,
                                                    vck[0], question)
                      for question in self.questions]
            voter.receive(return_code_tables=tables)

    def casting_phase(self, votes_per_voter=None):
...
def benchmark(n_voters, questions):
    ...
    # need the return value to assert if verification was successful
